...
class ProjectProperties(QDialog):
    ...
    def select_file(self):
        fileName = str(QFileDialog.getOpenFileName(self, 'Select Main File', self._item.path, '(*.py);;(*.*)'))
        if fileName != '':
            fileName = manage_files.convert_to_relative(self._item.path, fileName)[1:]
